...
@section('content')
<style>
    /* OTP slot inputs — exact clean style from the login page */
    .otp-slot {
        width: 44px;
        height: 54px;
        border-radius: 14px;
                ...
                @forelse ($recentLogins as $login)
                    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 rounded-2xl px-4 py-3 text-sm info-tile" data-login-row="{{ $login->id }}">
                        <div class="min-w-0 flex items-center gap-3">
                            <div class="nav-icon shrink-0">
                                <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><rect x="3" y="4" width="18" height="13" rx="2"/><path d="M8 20h8M12 17v3"/></svg>
                            </div>
                            <div class="min-w-0">
                                <p class="font-semibold flex items-center gap-2" style="color: var(--ink)">
                                    {{ \App\Models\LoginActivity::deviceLabel($login->user_agent) }}
                                ...
                                <p class="text-xs" style="color: #7a8580">{{ $login->location ?? 'Location unknown' }}</p>
                            </div>
                        </div>
...
<script>
    ...
    (function () {
        ...
        window.resetOtpSlots = function () {
            ...
            if (slotRow) slotRow.dataset.checking = 'false';
            if (status) {
